...
1a) Try running the game to make sure everything is prepped correctly
2) right click MSS -> properties -> Filesystem -> right click top node -> extract entire disc
3) create a folder "bin" inside \export_daes\
4) from the extracted disc data, copy both "dt_na.dat" and "main.dol" to the new folder \export_daes\bin\
5) cmd 
```cd export_daes
python export.py```
...
1a) (Blender 5.x and higher no longer support thus format! Use third party plugins or install an older versio nof blender, 4.5 and lower)
