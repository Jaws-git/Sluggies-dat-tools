...
Explanation found 2026-09-16 (Luigi membership edit, facial-posed neck vertices stretched to the floor): SK1/SK2 source arrays always mirror the skinned position buffer at `align32(struct arrays end) + gplVertexArr` (361/361 skinned models), including unused gap lines; the runtime depends on that beyond the per-entry pointers (facial poses on skinned vertices). The Peach hole was such a mirrored gap line. Rebuilt SKN now mirrors sources and `BlockValidator` rejects off-mirror sources — see PLAN_ModelReplacements.md 3.4, sixth bug note.
